---
permalink: /
title: "Rixin Zhou"
excerpt: "Computer vision, machine learning, and AI for cultural heritage"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

I am with the **School of Computer Science, Inner Mongolia University**. I work on **computer vision and machine learning**, with a particular interest in applying AI to cultural heritage. My recent work spans image-fragment matching and restoration, archaeological dating of Chinese bronzes, open-world recognition, and bronze-inscription recognition.

[View publications](/publications/){: .btn .btn--primary }
[Explore projects](/projects/){: .btn }

## Research interests

- Computer vision and representation learning
- Fine-grained and open-world recognition
- Image-fragment matching and restoration
- AI for archaeology and cultural heritage

## Education

- **2022.09–2026.06** — Doctoral degree in Computer Science and Technology, Jilin University
- **2019.09–2022.06** — Master's degree in Computer Technology, Jilin University
- **2012.09–2016.06** — Bachelor's degree in Automation, Jilin University

## Selected work

<div class="project-grid">
  <article class="project-card">
    <p class="project-card__meta">arXiv 2025</p>
    <h3><a href="https://arxiv.org/abs/2510.01651">LadderMoE</a></h3>
    <p>Ladder-side mixture-of-experts adapters for robust recognition of bronze inscriptions across heterogeneous visual domains.</p>
  </article>
  <article class="project-card">
    <p class="project-card__meta">ECCV 2024</p>
    <h3><a href="https://github.com/zhourixin/PairingNet">PairingNet</a></h3>
    <p>A learning-based pair-searching and pair-matching network for restoring image fragments.</p>
  </article>
  <article class="project-card">
    <p class="project-card__meta">CVPR 2023</p>
    <h3><a href="https://github.com/zhourixin/bronze-Ding">Archaeological Dating of Bronze Dings</a></h3>
    <p>A knowledge-guided relation graph for multi-granularity dating of Chinese bronze dings.</p>
  </article>
</div>

## Contact

For research questions or collaboration, email [zhourx22@mails.jlu.edu.cn](mailto:zhourx22@mails.jlu.edu.cn) or connect through [GitHub](https://github.com/zhourixin).
